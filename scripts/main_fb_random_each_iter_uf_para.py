...
while Ni < N:
    try:
        ...
        for i in range(1,dim_t):
            ...
            # #%%Random update of the parameters in UKF
            # for key, dist in par_dist_fx.items():
            #     par_kf_fx[key] = dist.rvs()
            # for key, dist in par_dist_hx.items():
            #     par_kf_hx[key] = dist.rvs()
            #%%Random update of the parameters in the true system
            for key, dist in par_dist_fx.items():
                par_true_fx[key] = dist.rvs()
                print(f"{key}_true: {par_true_fx[key]}")
            for key, dist in par_dist_hx.items():
                par_true_hx[key] = dist.rvs()
                print(f"{key}_true: {par_true_hx[key]}")
