...
while Ni < N:
    try:
        ...
        for i in range(1,dim_t):
            t_span = (t[i-1], t[i])
            dt_int = t_span[1] - t_span[0]
            w_plant_i = w_plant[i, :]
            for key, dist in par_dist_fx.items():
                par_true_fx[key] = dist.rvs()
            ...
            x_nom_ut = fx_ukf(x_post[:, i-1], None)
            fx_gen_Q = lambda si: (utils_fb.fx_for_UT_gen_Q(si, 
                                                           list_dist_fx_keys, 
                                                           t_span, 
                                                           x_post[:, i-1], 
                                                           ...
                                                           w_noise_kf)
                                    - x_nom_ut)
            fx_gen_Q2 = lambda si: utils_fb.fx_for_UT_gen_Q(si, 
                                                           list_dist_fx_keys, 
                                                           t_span, 
                                                           x_post[:, i-1], 
                                                           par_kf_fx.copy(),
            ...
            # print(f"x-bias: {mean_ut}")
            kf.Q = Q_ut
            fx_ukf_w_mean = lambda x, dt_kf: fx_ukf(x, dt_kf) + w_mean_ut*dt_int
            #Adaptive Q by LHS
            ...
            v_mean_ut, R_ut = ut.unscented_transformation(sigmas_hx, 
                                                        w_hx, 
                                                        fx = hx_gen_R)
            hx_ukf_w_mean = lambda x_in: hx_ukf(x_in) + v_mean_ut
            kf.R = R_ut
            # print(f"y-bias: {mean_ut}")
            ...
            kf.update(y[:, i], hx = hx_ukf_w_mean)
...
#%% MC plot of v:
N_mc = int(1e5)
y_nom = hx_ukf(x_prior)
par_mc_hx, fig_mc, ax_mc = utils_fb.get_mc_points(par_dist_hx, 
                                                N_mc_dist = N_mc, 
                                                plot_mc_samples=False,
                                                labels = list(par_true_hx.keys())
                                                             )
y = utils_fb.get_ys_by_MC(par_mc_hx, x_prior, dim_y) - y_nom
y_ukf = hx_ukf(x_prior) - y_nom
y_ukf_w_mean = hx_ukf_w_mean(x_prior) - y_nom
y_plant_para = utils_fb.hx(x_prior, par_true_hx) - y_nom 

fig, ax = plt.subplots(1,1)
ax.hist(y.T, bins = 50, alpha =.2)
ax.scatter(y_ukf, [10],label = "y_ukf")
ax.scatter(y_ukf_w_mean, [10], label = "y_ukf_w_mean")
ax.scatter(y.mean(), [10], label = "MC mean")
ax.scatter(y_plant_para, [10], label = "Plant parameter")
ax.legend()
