...
from . import utils_ukf_sqrt#.nearestPD as nearestPD
# from utils_ukf_sqrt import nearestPD
# from utils_ukf_sqrt import cholupdate

# import utils_ukf_sqrt#.cholupdate as cholupdate
...
def ukf_sqrt(y, x0, f, h, Q, R, u, P0=None, alpha=0.001, beta=2):
    ...
    for i in range(1, N):
        ...
        S = utils_ukf_sqrt.cholupdate(qr_R[np.ix_(ix, ix)], ex[:, 0], sgnW0)

        qr_Q, qr_R = scipy.linalg.qr( ey[:, 1:].T )
        Syy = utils_ukf_sqrt.cholupdate(qr_R[np.ix_(iy, iy)], ey[:, 0], sgnW0)
        Syy = Syy
        ...
        U = K*Syy.T
        for j in range(ny):
            S = utils_ukf_sqrt.cholupdate(S, np.ravel(U[:,j]), sgnW0)
            #print(S)
            #S = utils_ukf_sqrt.nearestPD(S)
            S[np.isnan(S)] = 0
            #S[np.isinf(S)] = 10000

        P[:,:,i] = S.T*S
        #P[:,:,i] = utils_ukf_sqrt.nearestPD(P[:,:,i])
