...
# N_LHS_list = np.array([10, 15, 20, 25])
mean_cost = {"x1": np.zeros((3, N_LHS_list.shape[0])),
             "x2": np.zeros((3, N_LHS_list.shape[0])),
             "x3": np.zeros((3, N_LHS_list.shape[0]))
             }
sigma_cost = {"x1": np.zeros((3, N_LHS_list.shape[0])),
             "x2": np.zeros((3, N_LHS_list.shape[0])),
             "x3": np.zeros((3, N_LHS_list.shape[0]))
             }
for q in range(N_LHS_list.shape[0]):
    ...
    for i in range(dim_x):
        cost = np.vstack([j_valappil[i], j_valappil_lhs[i], j_valappil_qf[i]]).T
...
if len(N_LHS_list) >= 2:
    ...
    labels_mu = [r"$\mu_{cost}^{GenUT}$", "$\mu_{cost}^{LHS}$", "$\mu_{cost}^{fixed}$"]
    for i in range(dim_x):
    ...
    fig_n_s, ax_n_s = plt.subplots(dim_x,1)
    # ax_n_s[0].set_xlabel(r"N_{LHS}")
    # ax_n_s[0].set_ylabel(r"\sigma_{cost}")
    labels_sigma = [r"$\sigma_{cost}^{GenUT}$", "$\sigma_{cost}^{LHS}$", "$\sigma_{cost}^{fixed}$"]
    for i in range(dim_x):
